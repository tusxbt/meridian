...
const toolMap = {
  ...
  update_config: ({ changes, reason = "" }) => {
    ...
    const CONFIG_MAP = {
      ...
      outOfRangeDownWaitMinutes: ["management", "outOfRangeDownWaitMinutes"],
  ...
  try {
    ...
    if (success) {
      if (name === "swap_token" && result.tx) {
        ...
        const _pool = _lastValidatedPoolDetail;
        notifyDeploy({
          ...
          tvl: _pool ? poolDetailTvl(_pool) : null,
          volume: _pool?.volume ?? _pool?.volume_window ?? null,
          feeActiveTvlRatio: _pool ? poolDetailFeeActiveTvlRatio(_pool) : null,
          organic: _pool?.organic_score ?? _pool?.token_x?.organic_score ?? null,
          smartMoney: _pool?.smart_money_buy ?? false,
          volatility: _pool ? poolDetailVolatility(_pool) : null,
