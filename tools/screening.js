...
function getRawPoolScreeningRejectReason(pool, s) {
  ...
  // volatility not required — pools without usable volatility data are still eligible
...
async function searchAssetsBySymbol(symbol) {
  const res = await fetch(`${DATAPI_JUP}/assets/search?query=${encodeURIComponent(symbol)}`, { signal: AbortSignal.timeout(FETCH_TIMEOUT_MS) });
  ...
  if (Object.keys(blockedDevs).length > 0) {
    ...
    if (missingDev.length > 0) {
      const devResults = await Promise.allSettled(
        missingDev.map((p) =>
          fetch(`${DATAPI_JUP}/assets/search?query=${p.base.mint}`, { signal: AbortSignal.timeout(FETCH_TIMEOUT_MS) })
...
export async function getTopCandidates({ limit = 10 } = {}) {
  ...
  log("screening", `After OKX filter: ${eligible.length} eligible`);
  ...
  if (config.indicators.enabled && eligible.length > 0) {
    ...
    for (const pool of eligible) {
      ...
      // Log as info only — indicator is a signal boost, not a hard filter
      if (confirmation && !confirmation.skipped) {
        const status = confirmation.confirmed ? "✅ confirmed" : "⚠️ not confirmed";
        log("screening", `Indicator ${status} for ${pool.name}: ${confirmation.reason}`);
      }
    }
  }
