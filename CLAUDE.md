...
| maxFeeActiveTvlRatio | screening | null (no cap) |
...
- Currently evolves `minFeeActiveTvlRatio` and `minOrganic` only — other keys (volatility caps, TVL bands) intentionally fixed
...
- DLMM SDK slippage parameter unit inconsistency: `addLiquidityByStrategyChunkable` uses percent (10 = 10%), `initializePositionAndAddLiquidityByStrategy` uses bps (1000 = 10%). Both intended for 10% — verify against SDK before changing.
