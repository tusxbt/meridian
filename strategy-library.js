...
const DEFAULT_STRATEGIES = {
  tuski_bidask: {
    ...
    range: {
      ...
      bins_below_min: 35,
      bins_below_max: 45,
      notes: "Tight bid-ask range — concentrates liquidity close to current price for maximum fee density per swap. At bin_step=80 and 45 bins, covers approximately -24% downside. Capital is dense near price = high fees when in range. Goes OOR faster — rely on fast OOR exit (10 min) and redeploy cycle to stay profitable.",
    },
    exit: {
      ...
      oor_wait_minutes: 10,
      notes: "Trailing TP activates at +2% peak PnL, exits when drops 1.5% from peak (15s reconfirmation). Hard stop if price drops -50% in the tracking window. Close if OOR for 10+ consecutive minutes — tight range goes OOR fast, don't wait. Close if yield < 7% fee/TVL per 24h after 25 min age — pool is dying. After 3 consecutive OOR closes on same pool → 12h cooldown on that pool and base token.",
    },
