...
const configuredMaxBinsBelow = numericConfig(u.maxBinsBelow)
  ?? (legacyBinsBelow != null ? Math.max(legacyBinsBelow, configuredMinBinsBelow) : 45);
...
export const config = {
  ...
  screening: {
    ...
    minTokenFeesSol:   u.minTokenFeesSol   ?? 20,  // global fees paid (priority+jito tips). below = bundled/scam
    ...
    minTokenAgeHours:   u.minTokenAgeHours   ?? 2, // 2 = minimum 2h token age
  ...
  management: {
    ...
    outOfRangeWaitMinutes:     u.outOfRangeWaitMinutes     ?? 1,  // upside OOR wait before close
    outOfRangeDownWaitMinutes: u.outOfRangeDownWaitMinutes ?? 1,  // downside OOR wait before close
    oorCooldownTriggerCount: u.oorCooldownTriggerCount ?? 5,
    oorCooldownHours:       u.oorCooldownHours       ?? 1,
    repeatDeployCooldownEnabled: u.repeatDeployCooldownEnabled ?? true,
    repeatDeployCooldownTriggerCount: u.repeatDeployCooldownTriggerCount ?? 5,
    ...
    collapseCheckMinAge:    u.collapseCheckMinAge    ?? 30,   // minutes before collapse rules activate
    volumeCollapseDropPct:  u.volumeCollapseDropPct  ?? 75,   // % drop in fee/TVL from peak → Rule 7
    tvlCollapseDropPct:     u.tvlCollapseDropPct     ?? 50,   // % drop in position value from peak → Rule 8 (in-range only)
  ...
  indicators: {
    ...
    entryPreset: indicatorUserConfig.entryPreset ?? "tuski_bidask",
    ...
    rsiOversold: indicatorUserConfig.rsiOversold ?? 25,
